...
Token_Kind :: enum {
    ...
    KW_STRUCT, KW_CONST, KW_LAYOUT, KW_LOCATION, KW_BINDING,
    KW_IN, KW_OUT, KW_UNIFORM, KW_FLAT, KW_PUSH_CONSTANT,
    KW_BUFFER,
...
keyword_set :: [] struct {kind:Token_Kind,str:string} {
    ...
    {.KW_LOCATION, "location"}, {.KW_BINDING, "binding"},   {.KW_IN, "in"},
    {.KW_OUT, "out"},           {.KW_UNIFORM, "uniform"},   {.KW_FLAT, "flat"},
    {.KW_PUSH_CONSTANT, "push_constant"}, {.KW_BUFFER, "buffer"},
};
...
make_lexer :: proc() -> Lexer {
    ...
    tokens = make_dynamic_array_len_cap([dynamic]^Token, 0, 1000);
...
lexer_make_token :: proc(using lexer: ^Lexer) -> ^Token {
    t := new(Token);
